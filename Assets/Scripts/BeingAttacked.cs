...
public class BeingAttacked : MonoBehaviour {
    ...
    void Start () {
        ...
        attackeds[0] = rb.gameObject.transform.GetChild(4).gameObject;
        attackeds[1] = rb.gameObject.transform.GetChild(5).gameObject;
        attackeds[2] = rb.gameObject.transform.GetChild(6).gameObject;
        attackeds[3] = rb.gameObject.transform.GetChild(7).gameObject;
    }
    ...
    {
        if (attMovement.isAttacking)
        { // During attack, player is invulnerable
            return;
        }
    ...
    IEnumerator BeAttacked(int i)
    {
        ...
        Sprite redOne = attackeds[i].GetComponent<SpriteRenderer>().sprite;
        rb.gameObject.GetComponent<Renderer>().enabled = false;
        yield return new WaitForSeconds(.1f);
        attackeds[i].GetComponent<SpriteRenderer>().sprite = hurts[i];
        yield return new WaitForSeconds(.1f);
        attackeds[i].GetComponent<SpriteRenderer>().sprite = redOne;
        yield return new WaitForSeconds(.1f);
        attackeds[i].GetComponent<SpriteRenderer>().sprite = hurts[i];
